...
class TestDFS:
    ...
    def test_bridges(self, graph_dfs):
        assert set(graph_dfs.bridges) == {(1, 3), (3, 4), (3, 5), (2, 11), (11, 12)}

    def test_articulation_points(self, graph_dfs):
        assert set(graph_dfs.articulation_points) == {1, 2, 3, 5, 11}

    def test_articulation_points_small_graph(self, small_graph_dfs):
        assert set(small_graph_dfs.articulation_points) == {16, 17}
    ...
    def test_links(self, small_graph_dfs):
        assert set(small_graph_dfs.tree_links) == {
            (16, 17),
            (17, 19),
            (19, 20),
            (20, 21),
...
class TestBFS:
    def test_shortest_path(self, graph_bfs):
        assert graph_bfs.shortest_path(12) == [0, 2, 11, 12]

        assert graph_bfs.shortest_path(8) == [0, 1, 3, 5, 8]

    def test_links(self, small_graph_bfs):
        assert set(small_graph_bfs.tree_links) == {
            (16, 17),
            (16, 18),
            (17, 20),
            (17, 19),
